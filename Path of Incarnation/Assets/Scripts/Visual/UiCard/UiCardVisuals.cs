...
public class UiCardVisuals : MonoBehaviour
{
    ...
    [SerializeField]
    private UIEffect disolveEffect;

    public MMF_Player HitFeedback => hitFeedback;
    ...
    [Button]
    public void PlayDissolveEffect(float duration = 1f)
    {
        if (disolveEffect == null)
            return;

        DOTween.Kill(disolveEffect); // prevent stacking tweens

        disolveEffect.transitionRate = 0f;

        DOTween.To(
            () => disolveEffect.transitionRate,
            x => disolveEffect.transitionRate = x,
            1f,
            duration
        )
        .SetTarget(disolveEffect)
        .SetEase(Ease.InOutCubic);

        availabilityLight.enabled = false;
