...
{
    private readonly List<UiZone> zones = new();
    public IReadOnlyList<UiZone> Zones => zones;

    // ---- Registration from Zone.OnEnable/OnDisable ----
    public void Register(UiZone z)
    {
        if (z && !zones.Contains(z)) zones.Add(z);
    }

    public void Unregister(UiZone z)
    {
        if (z) zones.Remove(z);
    }

    // ---- Queries ----
    public UiZone GetZoneOfType(ZoneType type) => zones.Find(z => z.zoneType == type);

    public List<UiZone> GetZonesOfType(ZoneType type)
    {
        var list = new List<UiZone>();
    ...
    public List<UiZone> GetValidDestinations(UiCard card)
    {
        var list = new List<UiZone>();
    ...
    public bool TryMoveCard(UiCard card, UiZone to, MoveType moveType, out string reason)
        => TryMoveCard(card, card?.CurrentZone, to, moveType, out reason);

    public bool TryMoveCard(UiCard card, UiZone from, UiZone to, MoveType moveType, out string reason)
    {
        ...
        EventBus.Publish(new BoardStateChangedEvent());
    ...
    public int AdvanceAllOneStep()
    {
        ...
        var pairs = new List<(UiCard card, UiZone from, UiZone to)>();
        foreach (var z in Zones)
        {
