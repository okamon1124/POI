...
public class UiZone : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    [Header("Ownership")]
    ...
    [Header("Flow")]
    [SerializeField] public UiZone nextZone;
    ...
    public event System.Action<UiZone> OccupantsChanged;
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        string reason;
        if (!ZoneManager.I.TryMoveCard(card, this, MoveType.Player, out reason))
        {
            Debug.Log($"Drop rejected: {reason}");
            return;
        }
    
        Debug.Log($"Move success: {card.name} moved to {zoneType}");
    }
